...
class Mutation:
    ...
    def add_syn(self, chrom):
        ...
        for row_index in range(num_nrns):
            if row_index not in output_nrns:
                for col_index in range(num_nrns):
                    # No self-loops
                    if col_index not in output_nrns and row_index != col_index and syn_matrix[row_index][col_index] == 0:
                        choices.append((row_index, col_index))
    ...
    def disc_nrn(chrom):
        ...
        choices = []
        for i in range(num_nrns):
            if i not in input_nrns and i not in output_nrns and i not in disconnected_nrns:
                choices.append(i)
        ...
        while choices:
            ...
            intersection = [x for x in pre_syn_nrns if x in post_syn_nrns]
            if not intersection:
